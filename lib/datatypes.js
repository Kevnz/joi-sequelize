...
module.exports = {
  ...
  DOUBLE: (precision, scale) =>
    new JSType({ type: "double", precision: precision, scale: scale }),
  DECIMAL: (precision, scale) =>
    new JSType({ type: "decimal", precision: precision, scale: scale }),
  GEOMETRY: (feature, srid) =>
    new JSType({ type: "geometry", feature: feature, srid: srid }),
  INTEGER: () => new JSType({ type: "integer" }),
  JSON: () => new JSType({ type: "json" }),
  JSONB: () => new JSType({ type: "jsonb" }),
  ENUM: function() {
    return new JSType({ type: "enum", values: _.map(arguments, e => e) });
  },
  FLOAT: (precision, scale) =>
    new JSType({ type: "float", precision: precision, scale: scale }),
  REAL: (precision, scale) =>
    new JSType({ type: "real", precision: precision, scale: scale }),
